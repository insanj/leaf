...
const getMasterListRugs = () => {
  const materials = acnh_master_list["Rugs"];
...
const generateRows = (searchText) => {
  const filtered = getMasterListRugs().filter(rug => {
...
export default function LeafShopRugsSection({ searchText, rows=generateRows(searchText) }) {
