...
const getMasterListFlooring = () => {
  const materials = acnh_master_list["Flooring"];
...
const generateRows = (searchText) => {
  const filtered = getMasterListFlooring().filter(flooring => {
    const lowercase = searchText.toLowerCase();
    if (!lowercase || lowercase.length < 1) {
      return true;
    }
    const searchable = Object.values(flooring).map(s => s ? s.toLowerCase() : '');
...
export default function LeafShopFlooringSection({ searchText, rows=generateRows(searchText) }) {
