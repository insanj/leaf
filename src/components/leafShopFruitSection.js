...
const getMasterListFruit = () => {
  const materials = acnh_master_list["Fruit"];
...
const generateRows = (searchText) => {
  const filtered = getMasterListFruit().filter(fruit => {
    const lowercase = searchText.toLowerCase();
    if (!lowercase || lowercase.length < 1) {
      return true;
    }
    const searchable = Object.values(fruit).map(s => s.toLowerCase());
...
export default function LeafShopFruitSection({ searchText, rows=generateRows(searchText) }) {
