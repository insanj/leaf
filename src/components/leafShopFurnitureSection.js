...
const getMasterListFurniture = () => {
  const materials = acnh_master_list["Furniture"];
...
const generateRows = (searchText) => {
  const filtered = getMasterListFurniture().filter(furniture => {
...
export default function LeafShopFurnitureSection({ searchText, rows=generateRows(searchText) }) {
  ...
  const sortRows = (toSort) => {
    ...
    let sorted = toSort.sort((r1, r2) => {
      ...
      } else if (columnLabel === 'Variations') {
        const v1 = r1.variations && r1.variations.length > 0 ? r1.variations : '';
        const v2 = r2.variations && r2.variations.length > 0 ? r2.variations : '';
